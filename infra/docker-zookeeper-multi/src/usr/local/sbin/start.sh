...
do
  if [[ $VAR =~ ^ZOOKEEPER_SERVER_[0-9]+= ]]; then
    ...
    if [ "${SERVER_ID}" = "${ZOOKEEPER_ID}" ]; then
      ...
      SERVER_IP_PORTS=`echo "$VAR" | sed 's/.*=//'`
      SERVER_IP=`echo "$(cut -d':' -f1 <<<"$SERVER_IP_PORTS")"`
      PORT_1=`echo "$(cut -d':' -f2 <<<"$SERVER_IP_PORTS")"`
      PORT_2=`echo "$(cut -d':' -f3 <<<"$SERVER_IP_PORTS")"`
      echo "server.${SERVER_ID}=${SERVER_IP}:${PORT_1}:${PORT_2}" >> /opt/zookeeper/conf/zoo.cfg
      echo "server.${SERVER_ID}=${SERVER_IP}:${PORT_1}:${PORT_2}"
    fi
  fi
done
